...
const GestionJeu = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [sectionsRes, gamesRes] = await Promise.all([
        api.get('/api/game/sections', { headers: { Authorization: `Bearer ${token}` } }),
        api.get('/api/game/games', { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const handleUpdateSection = async () => {
    ...
    try {
      ...
      if (editSection.image) formData.append('image', editSection.image);
  ...
  const handleAddGame = async () => {
    if (!newGame.name || !newGame.image || !newGame.url || !newGame.section) {
      setError('Tous les champs du jeu sont requis');
      return;
    }
    const token = localStorage.getItem('token');
    try {
  ...
  const handleUpdateGame = async () => {
    ...
    try {
      ...
      if (editGame.image) formData.append('image', editGame.image);
    ...
    <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center">
              <label htmlFor="section-image-input" className="cursor-pointer bg-blue-100 hover:bg-blue-200 text-blue-700 py-2 px-4 rounded-lg border-2 border-dashed border-blue-300 flex items-center transition-colors duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                {editSection?.image?.name || newSection?.image?.name || "Choisir une image"}
              </label>
              <input
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-8 border-4 border-orange-200 transform hover:scale-[1.01] transition-all duration-200">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center">
              <label htmlFor="game-image-input" className="cursor-pointer bg-blue-100 hover:bg-blue-200 text-blue-700 py-2 px-4 rounded-lg border-2 border-dashed border-blue-300 flex items-center transition-colors duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                {editGame?.image?.name || newGame?.image?.name || "Choisir une image"}
              </label>
              <input
                id="game-image-input"
                type="file"
      ...
      <div className="bg-white rounded-xl shadow-md p-6 border-4 border-purple-200">
        ...
        <div className="space-y-6">
          {sections.map((section) => (
            <div key={section._id} className="bg-gray-50 rounded-lg p-4 border-2 border-gray-200 hover:border-purple-300 transition-colors duration-200">
              ...
              <div className="ml-6 space-y-2">
                {games
                  ...
                  .map((game) => (
                    <div key={game._id} className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200">
