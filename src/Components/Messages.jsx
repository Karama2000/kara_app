...
const Messages = () => {
  ...
  const [unreadSenders, setUnreadSenders] = useState([]); // eslint-disable-line no-unused-vars
      ...
      try {
        ...
        filteredUsers = filteredUsers.map(user => ({
          ...
          imageUrl: user.imageUrl && !user.imageUrl.startsWith('http')
            ? `https://kara-back.onrender.com/Uploads/${user.imageUrl}`
            : user.imageUrl || 'https://via.placeholder.com/50',
  ...
  const fetchConversation = useCallback(async () => {
    ...
    try {
      ...
      const unreadMessages = messagesResponse.data.filter(msg => msg.recipient._id === userId && !msg.read);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const updatedMessages = newMessages.map(msg => ({
        ...msg,
        sender: { _id: userId, prenom: localStorage.getItem('prenom'), nom: localStorage.getItem('nom'), role },
  ...
  const groupedUsers = filteredUsers.reduce((acc, user) => {
    if (role === 'enseignant' && user.role === 'eleve') {
      ...
      const group = user.role === 'enseignant' ? 'Enseignants' : 'Parents';
  ...
  <div className="messages-container bg-gradient-to-br from-blue-50 to-pink-50 min-h-screen p-4 md:p-6">
    ...
    <header className="flex items-center justify-between bg-white shadow-lg p-4 rounded-2xl mb-6 backdrop-blur-sm bg-opacity-80">
      ...
      >
        ...
        <span className="font-medium">          🏠 Retour
</span>
      </button>
      ...
      <div className="w-full lg:w-1/3 bg-white rounded-2xl p-5 shadow-lg border border-gray-100">
        ...
        {role === 'enseignant' && (
          <div className="mb-5 space-y-3">
            ...
            <div className="relative">
              <FontAwesomeIcon icon={faUsers} className="absolute left-3 top-3 text-pink-400" />
              <select
                value={classe}
                onChange={(e) => setClasse(e.target.value)}
                className="w-full pl-10 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-pink-300 focus:border-pink-400 appearance-none bg-white"
                disabled={!niveau}
              >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {groupedUsers[group][subGroup].map(user => (
                        <div
                          ...
                          className={`cursor-pointer border rounded-xl px-3 py-2 flex items-center gap-3 shadow-sm transition-all hover:shadow-md w-full ${
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <img
                    src={users.find(u => u._id === recipientId)?.imageUrl}
                    alt="Recipient"
                ...
                <div className="space-y-4 mb-6 overflow-y-auto max-h-[50vh] pr-2 scrollbar-thin scrollbar-thumb-blue-200 scrollbar-track-blue-50">
                  {messages.length === 0 ? (
                    ...
                    messages.map(message => (
                      ...
                      >
                        <div
                          className={`p-4 rounded-2xl max-w-[80%] lg:max-w-[60%] shadow-sm ${
